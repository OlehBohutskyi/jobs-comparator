...
class RequirementsAnalysis(Base):
    ...
    id = Column(Integer, primary_key=True)
    domains = Column(String(255), nullable=False)  # Comma-separated list of domains
    top_words = Column(Text, nullable=True)  # JSON string of words and their frequencies
    summary = Column(Text, nullable=True)  # Summary generated by ChatGPT
    created_at = Column(DateTime, server_default=func.now())
    is_educational_analysis = Column(Boolean, default=False)  # Flag for educational program analysis
    education_program_file = Column(String(255), nullable=True)  # Path to uploaded program file
    education_program_filename = Column(String(255), nullable=True)  # Original filename
    education_program_filetype = Column(String(50), nullable=True)  # File type (pdf, docx, txt)
    education_program_text = Column(Text, nullable=True)  # Extracted text from the file
    education_program_analysis = Column(Text, nullable=True)  # Analysis comparing job requirements with program
